...
locals {
  ...
  # 1. Definição do mapeamento exclusivo de cada VM: [IP, vmstartcount, staticvmname]
  vm_matrix = {
    "PSAPK01" = ["192.168.121.11", 1, "PSAPK01"]
    "PSAPK04" = ["192.168.121.13", 4, "PSAPK04"]
  }
}
...
inputs = {
  vm = {
    for name, config in local.vm_matrix : name => {
      staticvmname      = config[2]
      network           = { "PG_Servico" = [config[0]] }
      vmstartcount      = config[1]
      
      # Valores padrão replicados automaticamente para todas as instâncias (Bloco K)
